...
Data Source:
    - artifacts/model.joblib (trained in the notebook or app_femi.py)

Terminal commands to run this service from the root project folder:

uv run fastapi dev src/mlstudio/serve_femi.py      # development (auto-reload)
uv run fastapi run src/mlstudio/serve_femi.py      # production

- OR -

uv run uvicorn mlstudio.serve_femi:app --reload    # development (auto-reload)
uv run uvicorn mlstudio.serve_femi:app             # production
...
if not MODEL_PATH.exists():
    ...
    raise FileNotFoundError(
        ...
        "Run the training notebook or app_femi.py first."
    )
